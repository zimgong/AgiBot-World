...
class DatasetArguments(BaseDatasetArguments):
    ...
    data_root_dir: Optional[List[str]] = field(
        default_factory=lambda: [
            "/home/data/zimgong/lerobot_lift_visual_processed_280",
        ],
    )
...
class GOModelArguments(BaseModelArguments):
    ...
    decoder_type: str = "mean_flow"  # Use MeanFlow instead of DDPM
    dispersive_use: bool = True  # Enable dispersive regularization
    dispersive_weight: float = 0.5  # Weight for dispersive loss
    dispersive_loss_type: str = "infonce_cos"  # Type of dispersive loss
    dispersive_temperature: float = 0.3  # Temperature for InfoNCE
    dispersive_margin: float = 0.5  # Margin for hinge loss
    dispersive_target: list = field(default_factory=lambda: ["Cond"])
